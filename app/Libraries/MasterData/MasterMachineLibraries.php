...
class MasterMachineLibraries
{
	...
	public function add_or_update($request)
	{
		...
		if (isset($id) && $id != '') {
			...
			if ($machine_find->Symbols != $request->Symbols || $machine_find->Note != $request->Note) {
				// dd('1');
			...
			if ($key > 1) {
				if (!is_null($value[2]) && strlen($name) <= 25) {
					if (is_numeric($stockmin) && $stockmin > 0) {
						...
						if ($machine) {
							if ($machine->Stock_Min != $value[4] || $machine->Stock_Max != $value[3] || $machine->MAC != $mac) {
								MasterMachine::where('IsDelete', 0)->where('ID', $machine->ID)->update([
									'Stock_Min'        => $value[4],
									'Stock_Max'        => $value[3],
								]);
								$find_mac = MasterMachine::where('IsDelete', 0)->where('ID', '<>', $machine->ID)->where('MAC', $mac)->first();
								if ($find_mac && $mac) {
									array_push($err, __('Mac') . ' ' . __('Does Exist') . ' ' . __('In') . ' ' . __('Position') . ' ' . ($key + 1));
								} else {
									MasterMachine::where('IsDelete', 0)->where('ID', $machine->ID)->update([
										'MAC'        => $value[5],
									]);
							...
							$find_mac = MasterMachine::where('IsDelete', 0)->where('MAC', $mac)->first();
							if ($find_mac && $mac) {
								array_push($err, __('Mac') . ' ' . __('Does Not Exist') . ' ' . __('In') . ' ' . __('Position') . ' ' . ($key + 1));
							} else {
								$machine = MasterMachine::create([
									'Name'             => $value[2],
									'Symbols'          => $value[2],
