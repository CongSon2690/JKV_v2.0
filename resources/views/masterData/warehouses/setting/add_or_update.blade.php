									...
									<div class="area_select col-6">
										<select name="Area" class="form-control select2" >
											...
											@foreach($area as $value)
												<option value="{{$value->ID}}" {{old('Area') ? (old('Area') == $value->ID ? 'selected' : '') : ($warehouse ? ($warehouse->Area == $value->ID ? 'selected' : '') : '')}} class="">
													{{$value->Name}}
												</option>
				                    		@endforeach
				                    </select>
									</div>
									<div class="area_input col-6 ">
										<input type="text"  class="form-control area_input " id="Area" value="" name="Area1" placeholder="{{__('Enter')}} {{__('Area')}}">
									</div>
									<button type="button" class="btn btn-success float-right col-6 btn-input">{{__('Create')}} {{__('Area')}}</button>
									<button type="button" class="btn btn-success float-right col-6 btn-select">{{__('Choose')}} {{__('Area')}}</button>
				                </div>
		                </div>
