                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="col-md-2" style="float:right;">
                            <a href="{{ route('warehousesystem.import.location', ['Format' => $request->Format]) }}"
                                class="btn btn-warning">{{ __('Diagram') }}</a>
                                <a href="{{ route('masterData.stock.export_file', [
                                ...
                                <div class="form-group col-md-2">
                                    ...
                                    <select class="custom-select select2" name="location">
                                        ...
                                        @foreach ($list_location as $key => $value)
                                            <option value="{{ $value->ID }}"
                                                {{ $request->location == $value->ID ? 'selected' : '' }}>
                                                {{ $value->Symbols }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-md-2">
                                    ...
                                    <select class="custom-select select2" name="warehouse">
                                        ...
                                        @foreach ($list_ware as $key => $value)
                                            <option value="{{ $value->ID }}"
                                                {{ $request->warehouse == $value->ID ? 'selected' : '' }}>
                                                {{ $value->Name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                @foreach ($data as $value)
                                    ...
                                    @foreach ($value['Inven'] as $value1)
                                        @foreach ($value1['Inven'] as $value2)
                                            <tr>
                                                <th rowspan="{{ count($value2['Inven']) + 2 }}"
                                                    style="vertical-align: middle;">{{ $value1['Symbols'] }}
                                                </th>
                                                <td rowspan="{{ count($value2['Inven']) + 2 }}"
                                                    style="vertical-align: middle;">{{ $value2['Pallet_ID'] }}
                                                </td>
                                                <td rowspan="{{ count($value2['Inven']) + 2 }}"
                                                    style="vertical-align: middle;">{{ $value2['Materials'] }}
                                                </td>
                                                <td rowspan="{{ count($value2['Inven']) + 2 }}"
                                                    style="vertical-align: middle;">{{ $value2['Supplier'] }}
                                                </td>
                                            </tr>
                                            <?php $sum = 0;
                                            $dem = 0; ?>
                                            @foreach ($value2['Inven'] as $value2)
                                                <tr>
                                                    ...
                                                    <td>{{ $value2->Box_ID }}</td>
                                                    <td>{{ $request->Format == 1 ?  floatval($value2->Inventory) :  floatval($value2->Quantity) }}</td>
                                                    {{-- <td>{{ $value2->materials ? ($value2->materials->unit ? $value2->materials->unit->Symbols : '') : ($value2->product->unit ? $value2->product->unit->Symbols : '') }} --}}
                                            ...
                                            <tr>
                                                <td colspan="2" style="background-color: #ccffff;">
                                                    {{ __('Roll Number') }} : {{ $dem }}</td>
                                                <td colspan="2" style="background-color: #ccffff;">
                                                    {{ __('Quantity') }}(kg) : {{ floatval($sum) }}</td>
                                            </tr>
                                        @endforeach
                                    @endforeach
                                    <tr>
                                        <td colspan="4" style="background-color: #99ff99;">{{ __('Roll Number') }} :
                                            {{ $dem_t }}</td>
                                        <td colspan="4" style="background-color: #99ff99;">{{ __('Quantity') }}(kg) :
                                            {{ floatval($sum_t) }}</td>
                                    </tr>
                                @endforeach
                            </tbody> 
