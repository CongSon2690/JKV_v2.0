					...
					<div class="mold  card-body">
						<div class="row">
							<div class="form-group col-md-2 ">
									<label for="Mold">{{__('Product')}}</label>
									<div class="input-group">
									<select class="custom-select select2 mold" name="mold" id="Mold">
										<option value="">
							...
							<div class="form-group col-md-2 ">
									<label for="Mold">{{__('Quantity')}}</label>
									<div class="input-group">
						...
						<table class="table table-striped table-hover table-mold">
							<thead>
								<th  class="d-none">ID {{ __('Product') }} </th>
							...
							<tbody>
								@foreach($data->where('Product_ID','>',0) as $value)
									<tr class="tr-mater-{{$value->product ? $value->product->Symbols : '' }}">
										<td class="d-none">{{$value->product ? $value->product->Symbols : '' }}
										<input type="Text" value="{{$value->product ? $value->product->Symbols : '' }}" class="form-control d-none" id="mold" name="mold[]" >
										</td>
										<td>{{$value->product ? $value->product->Symbols : '' }}</td>
										<td><input type="number" value="{{floatval($value->Quantity_Product)}}" step="0.0001" class="form-control" id="Quantity_Product" name="Quantity_Product[]" ></td>
										<td>{{$value->user_created ? $value->user_created->name : '--' }}</td>
										<td>{{$value->Time_Created}}</td>
										<td>
											<button id="{{$value->product ? $value->product->Symbols : '' }}" class="btn btn-danger btn-delete-mater" style="width: 80px">
												{{__('Delete')}}
											</button>
										</td>
									</tr>
						...
						<div class="row">
							<div class="form-group col-md-2 ">
								...
								<div class="input-group">
									...
									<select class="custom-select select2 materials" name="materials" id="Materials">
										<option value="">
											{{__('Choose')}} {{__('Symbols')}}
										</option>
							...
							<div class="form-group col-md-2 ">
								<label for="Materials">{{__('Usage/PCS')}}</label>
								<div class="input-group">
			...
			if($.inArray(mold_id, arr_mold ) == '-1' && mold_id.length > 0 && cavity != '' && cycle_time != '' )
			{
				...
				$('.table-mold tbody').append(`
					<tr class="tr-mold-`+mold_id+`">
						...
						<td>`+mold_symbols+`</td>
						<td><input type="number" value="`+cycle_time+`" min ="0" step="0.0001" class="form-control" id="Quantity_Product" name="Quantity_Product[]" ></td>
						<td></td>
						<td></td>
						<td>
