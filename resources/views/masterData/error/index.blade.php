		...
		<div class="card-bordy p-3">
				<div class="h-space">
					<div class="form-group w-20">
						<label>{{__('Symbols')}} {{ __('Error') }}</label>
					...
					@if(Auth::user()->checkRole('create_master') || Auth::user()->level == 9999)
                        <a href="{{ route('masterData.error.show') }}" class="btn btn-success w-10">
							{{__('Create')}}
						</a>
					@endif
				</div>
			<table class="table table-error table-bordered table-striped text-nowrap w-100">
            </table>
		</div>
	</div>
@endsection
@push('scripts')
	<script src="{{ asset('js/master-data/error.js') }}"></script>
@endpush