...
module.exports = {
    async index(req, res) {
        ...
        let   productionLogs = productionLog().where('Master_Machine_ID', '=', selectedMachine)
                                          .where('Time_Created', '>=', startDate)
                                          .where('Time_Updated', '<=', endDate)

        if(Number(selectedShift)) productionLogs.where('Master_Shift_ID', '=', selectedShift)
    ...
    async detail(req, res) {
        ...
        let   quantity = 0, ng = 0
        let   productionLogs = productionLog().where('Master_Machine_ID', '=', machineId)
                                              .where('Time_Created', '>=', getStartOfDay().format('YYYY-MM-DD HH:mm:ss').toString())
                                              console.log(type)
