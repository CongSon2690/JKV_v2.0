...
module.exports = {
        ...
        if(planIsRunning.length) {
            for(const plan of planIsRunning) {
                ...
                const bom = await masterBom().where('Product_BOM_ID', '=', plan.Product_ID)
                                             ...
                                             .first()
                const product = await masterProduct().where('ID', '=', plan.Product_ID)
                                                     .first()
                products.push(product)
                                                     
                p += Math.floor((current.diff(startTime, 'seconds', true) / bom.Cycle_Time) * bom.Cavity)
    ...
    async initRuntime(req, res) {
        ...
        const runtime = await runtimeHistory().where('Master_Machine_ID', '=', machineId)
                                              .where('Time_created', '>=', moment().startOf('day').format('YYYY-MM-DD HH:mm:ss').toString())
                                              .get()
